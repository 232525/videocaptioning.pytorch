...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, ix):
        ...
        for dir in self.feats_dir:
            fc_feat.append(np.load(os.path.join(dir, 'video%i.npz' % (ix)))['feat'])
        """
        video_id = self.splits[self.mode][ix]
        fc_feat = []
        for dir in self.feats_dir:
            fc_feat.append(np.load(os.path.join(dir, '%s.npz' % (video_id)))['feat'])
        
        fc_feat = np.concatenate(fc_feat, axis=1)
        if self.with_c3d == 1:
            # c3d_feat = np.load(os.path.join(self.c3d_feats_dir, 'video%i.npz'%(ix))['feat'])
            c3d_feat = np.load(os.path.join(self.c3d_feats_dir, '%s.npz'%(video_id))['feat'])
        ...
        captions = self.captions[video_id]['final_captions']
        ...
        # gts在训练和测试中都用不到，可以不用返回
        # TODO：
        # 训练过程中，对于每一个视频，收集其所有的gts用于训练，则需要保留gts，并需要对fc_feats进行复制
