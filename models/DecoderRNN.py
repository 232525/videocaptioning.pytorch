...
class DecoderRNN(nn.Module):
    ...
    def forward(self,
        ...
        if mode == 'train':
            ...
            # 第一步 <sos>
            wt = torch.LongTensor([self.sos_id] * batch_size).cuda()
            unfinished = wt.eq(wt)
            for t in range(self.max_length - 1):
                # Decoder前向计算
                xt = self.embedding(wt)
                # 在帧特征上计算注意力
                ...
                if sample_max:
                    # greedy 
                    sampleLogprobs, wt = torch.max(logprobs, 1)
                else:
                    ...
                    sampleLogprobs = logprobs.gather(1, wt)

                # 生成单词index
                wt = wt.view(-1).long()
                unfinished = unfinished * (wt != self.eos_id)
                seq_preds.append(wt.view(-1, 1))
